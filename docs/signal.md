...
Signal is disabled by default. Leave the Signal variables unset unless you want to enable this transport.

To enable it, set all of the following in `.env`:
...
`SIGNAL_PROFILE=signal` is the Compose opt-in. Without it, the Signal services are assigned to an inactive profile and normal `docker compose up` does not start either Signal container.
...
After setting the Signal variables above:
...
If `SIGNAL_API_URL`, `SIGNAL_NUMBER`, or `SIGNAL_ALLOWED_SENDERS` is missing, the Signal CLI exits with a configuration error instead of running partially configured. These settings are not required by the other agent commands or services.
